...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{heart =}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{'heart.csv'}\NormalTok{)}
\NormalTok{heart$heart.disease =}\StringTok{ }\KeywordTok{factor}\NormalTok{(heart$heart.disease)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tree =}\StringTok{ }\KeywordTok{rpart}\NormalTok{(heart.disease~., }\DataTypeTok{data=}\NormalTok{heart)}
\KeywordTok{prp}\NormalTok{(tree)}
\end{Highlighting}
\end{Shaded}

\includegraphics{heartDiseaseDecisionTree_files/figure-latex/unnamed-chunk-4-1.pdf}
